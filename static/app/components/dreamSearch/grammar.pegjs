...
search
  = terms:(boolean_term / paren_term / search_term)* {
      return terms.flat();
    }

boolean_term = spaces boolean_operator spaces
...
paren_term
  = spaces open_paren spaces
    inner:(paren_term / boolean_term / search_term)+
    spaces closed_paren spaces {
      return tc.tokenLogicGroup(inner.flat());
    }

search_term
  = key_val_term / free_text_quoted / free_text

free_text
  = (spaces !key_val_term !boolean_operator (free_parens / [^()\n ]+) spaces)+ {
      return tc.tokenFreeText(text(), false);
    }

free_text_quoted
  = spaces value:quoted_value spaces {
      return tc.tokenFreeText(value.value, true);
    }

free_parens
  = spaces open_paren spaces free_text? spaces closed_paren spaces

// All key:value filter types
key_val_term
  = spaces key_val_filter spaces
...
has_filter
  = negation:negation? has:"has" sep value:(search_key / search_value) {
      const key = tc.tokenKeySimple(() => ({}), () => has, has, false);
      return tc.tokenFilter(FilterType.Has, key, value, TermOperator.Default, !!negation);
    }

// is filter. Specific to issue search
is_filter
  = negation:negation? is:"is" sep value:search_value {
      const key = tc.tokenKeySimple(() => ({}), () => is, is, false);
      return tc.tokenFilter(FilterType.Has, key, value, TermOperator.Default, !!negation);
    }
